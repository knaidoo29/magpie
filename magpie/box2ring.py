...
class Box2Ring:
    ...
    def get_weights(self, mc_size=10000, verbose=True):
        """Calculates Monte Carlo pixel weights for remapping from cartesian grid to polar coordinate grid.

        Parameters
        ----------
        mc_size : int
            Size of the Monte Carlo random points for calculating weights from Monte Carlo integration.
        verbose : bool
        ...
        for i in range(0, len(self.r2d)):
            ...
            if verbose == True:
                progress.progress_bar(i, len(self.r2d), explanation='Calculating weights')
            ...
            for j in range(0, len(self.r2d[0])):
                if w is None:
                    ...
                    if np.sum(self.ind_ws[i, j]) != 0.:
                        ...
                        f_polar[i, j] = np.nan
            if verbose == True:
                progress.progress_bar(i, len(self.r2d), explanation='Remapping')
        return f_polar
