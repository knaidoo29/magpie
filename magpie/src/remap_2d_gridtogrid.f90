...
subroutine remap_2d_gridtogrid(d1, x, y, boxsize, grid1, grid2, sperpix, d2)
  ...
  !   Size of the box.
  ! grid1 : int
  !   Size of the input 2D grid along one axis.
  ! grid2 : int
  ...
  real(kind=dp), intent(in) :: d1(grid1*grid1), x(grid2*grid2), y(grid2*grid2), boxsize
  real(kind=dp), intent(out) :: d2(grid2*grid2)

  real(kind=dp) :: dx1, dx2, xperpix_edges(sperpix+1), xperpix(sperpix)
  real(kind=dp) :: mhdx2, phdx2
  integer :: i, ixp, iyp, ixconv, iyconv, iconv
  ...
  do i = 1, grid2*grid2
    ...
    do ixp = 1, sperpix
      do iyp = 1, sperpix
        ...
        iconv = iyconv + grid1*ixconv + 1
        d2(i) = d2(i) + d1(iconv)
      end do
    end do

    d2(i) = d2(i)/(FLOAT(sperpix)**2.)
  end do


end subroutine remap_2d_gridtogrid
