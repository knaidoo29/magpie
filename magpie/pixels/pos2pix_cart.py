...
def pos2pix_cart1d(x, length, ngrid, origin=0.):
    ...
    if utils.isscalar(x) is True:
        ...
        pixID = src.which_pix_id_scalar(x, origin, dx)
    else:
        # pixID = src.which_pix_id_array(x=x, xmin=origin, dx=dx, xlen=len(x))
        pixID = src.which_pix_id_array(x, origin, dx)
    return pixID
...
def pos2pix_cart2d(x, y, lengths, ngrids, origin=[0., 0.]):
    ...
    if utils.isscalar(x) is True:
        ...
        pixID = src.pix_id_1dto2d_scalar(xpixID, ypixID, _ngrids[1])
    else:
        # pixID = src.pix_id_1dto2d_array(xpix_id=xpixID, ypix_id=ypixID, xlen=len(x), ygrid=_ngrids[1])
        pixID = src.pix_id_1dto2d_array(xpixID, ypixID, _ngrids[1])
    return pixID
...
def pos2pix_cart3d(x, y, z, lengths, ngrids, origin=[0., 0., 0.]):
    ...
    if utils.isscalar(x) is True:
        ...
        pixID = src.pix_id_1dto3d_scalar(xpixID, ypixID, zpixID, _ngrids[1], _ngrids[2])
    else:
        # pixID = src.pix_id_1dto3d_array(xpix_id=xpixID, ypix_id=ypixID, zpix_id=zpixID, xlen=len(x), ygrid=_ngrids[1], zgrid=_ngrids[2])
        pixID = src.pix_id_1dto3d_array(xpixID, ypixID, zpixID, _ngrids[1], _ngrids[2])
    return pixID
