...
def usphere_area(phi_min, phi_max, theta_min, theta_max):
    ...
    assert phi_min >= 0. and phi_min < 2.*np.pi, \
        "phi_min must lie within [0, 2pi]."
    assert phi_max > 0. and phi_max <= 2.*np.pi, \
        "phi_max must lie within [0, 2pi]."
    assert phi_min != phi_max, "phi_min cannot equal phi_max."
    assert theta_min >= 0. and theta_min < np.pi, \
        "theta_min must lie within [0, pi]."
    assert theta_max > 0. and theta_max <= np.pi, \
        "theta_max must lie within [0, pi]."
