...
def sky_rotate(phi, theta, phi_end, theta_end, phi_start=0., theta_start=0.):
    ...
    x_start, y_start, z_start = transforms.sphere2cart(r_start, phi_start, theta_start)
    u = np.array([x_start, y_start, z_start])
    # determine vector of end point but first convert to cartesian coordinates
    x_end, y_end, z_end = transforms.sphere2cart(r_start, phi_end, theta_end)
    ...
    if np.isscalar(phi) == True:
        ...
        r = np.ones(len(phi))
    # convert points on the sphere to cartesian coordinates
    x, y, z = transforms.sphere2cart(r, phi, theta)
    ...
    r_new, phi_new, theta_new = transforms.cart2sphere(x_new, y_new, z_new)
